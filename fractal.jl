...
function is_interesting(grid, max_iter; entropy_thresh=3.0, gradient_thresh=0.5, low_thresh=0.1, high_thresh=0.9)
...
heatmap(xs, ys, mandelbrot_set,
    size = (3360, 800),  # Force the output size to 3360 x 800 pixels
    xlims = (x_min, x_max),
    ylims = (y_min, y_max),
    color = cmap,
    axis = nothing,
